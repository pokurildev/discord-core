...
class Leveling(commands.Cog):
    def __init__(self, bot):
        ...
        self.bg_path = os.path.join("assets", "welcome_bg.png")

    def get_xp_for_level(self, level):
        """Формула: XP_needed = 5 * (lvl ^ 2) + 50 * lvl + 100"""
        if level < 0: return 0
        return 5 * (level ** 2) + 50 * level + 100

    def calculate_level(self, total_xp):
        """Рассчитывает текущий уровень на основе общего XP"""
        lvl = 0
        while total_xp >= self.get_xp_for_level(lvl):
            total_xp -= self.get_xp_for_level(lvl)
            lvl += 1
        return lvl, total_xp # Возвращает уровень и остаток опыта в текущем уровне

    async def update_user_xp(self, user_id, xp_to_add):
        async with await get_db() as db:
            # Пытаемся получить текущий XP
    ...
    async def on_message(self, message):
        ...
        if new_level > old_level:
            ...
            # Тут можно добавить логику выдачи ролей
    ...
    async def rank(self, interaction: discord.Interaction, member: discord.Member = None):
        ...
        if os.path.exists(self.bg_path):
            bg_image = Editor(self.bg_path).resize((900, 250), crop=True)
